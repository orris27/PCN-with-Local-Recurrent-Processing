...
class ClassifierModuleFirst(nn.Module):
    def __init__(self, in_channel_block, num_classes, hidden_channel):
        ...
        self.conv1 = nn.Conv2d(in_channel_block, in_channel_block * 2, kernel_size=3, stride=2, padding=1, bias=False)
        self.conv2 = nn.Conv2d(in_channel_block * 2, in_channel_block * 4, kernel_size=3, stride=2, padding=1, bias=False)
        #self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)

        self.linear_h = nn.Linear(in_channel_block * 4, hidden_channel, bias=False)
    ...
    def forward(self, x_block):
        x = self.conv2(self.conv1(x_block))
        out_block = F.avg_pool2d(x, x.size(-1)) # 1
...
class ClassifierModuleMiddle(nn.Module):
    def __init__(self, in_channel_block, in_channel_clf, num_classes, cls, hidden_channel, score_layer=False):
        ...
        #self.BN = nn.BatchNorm2d(in_channel_block)
        #self.se = SELayer(in_channel_block, 16)
        self.conv = nn.Conv2d(in_channel_block, in_channel_block * 2, kernel_size=3, stride=2, padding=1, bias=False)

        self.linear_h = nn.Linear(in_channel_block * 2 + in_channel_clf, hidden_channel, bias=False)
        self.linear = nn.Linear(hidden_channel, num_classes)
        ...
        if self.cls != 0:
            ...
            self.linear_bw = nn.Linear(hidden_channel, in_channel_block * 2 + in_channel_clf, bias=False)
        self.BN1d = nn.BatchNorm1d(hidden_channel)


    def forward(self, x_block, x_clf):
        #out_block = F.avg_pool2d(self.conv(x_block), x_block.size(-1))
        x = self.conv(x_block)
        out_block = F.avg_pool2d(x, x.size(-1))
        out_block = out_block.view(out_block.size(0), -1) # (batch_size, c_block)
...
class ClassifierModuleLast(nn.Module):
    def __init__(self, in_channel_block, in_channel_clf, num_classes, cls):
        ...
        if self.cls != 0:
            ...
            self.linear_bw = nn.Linear(num_classes, in_channel_block + in_channel_clf, bias=False)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, cls=0, ge=False):
        ...
        self.classifiers.append(ClassifierModuleFirst(in_channel_block=16, hidden_channel=64, num_classes=num_classes))
        self.classifiers.append(ClassifierModuleMiddle(in_channel_block=32, in_channel_clf=64, num_classes=num_classes, hidden_channel=80, cls=self.cls))
        self.classifiers.append(ClassifierModuleLast(in_channel_block=64, in_channel_clf=80, num_classes=num_classes, cls=self.cls))
...
def resnet56(num_classes, cls, ge):
    return ResNet(BasicBlock, [9, 9, 9], num_classes, cls, ge)
