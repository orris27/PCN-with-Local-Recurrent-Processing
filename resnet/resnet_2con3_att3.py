...
from attention import LinearLayer, SELayer, ScanLayer
...
class ClassifierModuleMiddle(nn.Module):
    def __init__(self, in_channel_block, in_channel_clf, num_classes, cls, hidden_channel, attention_1, attention_2):
    ...
    def forward(self, x_block, x_clf):
        ...
        out_block = F.avg_pool2d(self.attention_1(x_block), x_block.size(-1))
        out_block = out_block.view(out_block.size(0), -1) # (batch_size, c_block)
        out_clf = self.attention_2(x_clf) # (batch_size, c_clf)
...
class ClassifierModuleLast(nn.Module):
    def __init__(self, in_channel_block, in_channel_clf, num_classes, cls, attention):
        ...
        if attention == 'yes':
            self.attention = LinearLayer(in_channel_clf)
        elif attention == 'no':
            self.attention = lambda x:x # dummy
        else:
            raise ValueError
    ...
    def forward(self, x_block, x_clf):
        ...
        out_block = out_block.view(out_block.size(0), -1) # (batch_size, c_block)
        out_clf = self.attention(x_clf) # (batch_size, c_clf)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, cls=0, ge=False, attention_1=None, attention_2=None):
        ...
        self.classifiers.append(ClassifierModuleFirst(in_channel_block=16, hidden_channel=16, num_classes=num_classes, attention=attention_1))
        self.classifiers.append(ClassifierModuleMiddle(in_channel_block=32, in_channel_clf=16, num_classes=num_classes, hidden_channel=32, cls=self.cls, attention_1=attention_1, attention_2=attention_2))
        self.classifiers.append(ClassifierModuleLast(in_channel_block=64, in_channel_clf=32, num_classes=num_classes, cls=self.cls, attention=attention_2))
...
def resnet56(num_classes, cls, ge, attention_1, attention_2='yes'):
    print('*'*30)
    print('IMPORTANT: attention 2 is default to yes!!!!!!')
    print('*'*30)
    return ResNet(BasicBlock, [9, 9, 9], num_classes, cls, ge, attention_1, attention_2)
