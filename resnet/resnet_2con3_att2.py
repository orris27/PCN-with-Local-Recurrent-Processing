...
from attention import LinearLayer
...
class ClassifierModuleFirst(nn.Module):
    ...
    def forward(self, x_block):
        out_block = F.avg_pool2d(x_block, x_block.size(-1)) # 1
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, cls=0, ge=False, attention=None):
        ...
        self.classifiers.append(ClassifierModuleFirst(in_channel_block=16, hidden_channel=16, num_classes=num_classes))
        ...
        self.classifiers.append(ClassifierModuleLast(in_channel_block=64, in_channel_clf=32, num_classes=num_classes, cls=self.cls, attention=self.attention))
