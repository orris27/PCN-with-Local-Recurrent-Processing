...
class PredNetBpD(nn.Module):
    def __init__(self, num_classes=10, cls=0, Tied = False):
        ...
        # Classifier
        self.FFconv = nn.Conv2d(self.ocs[-1], 64, kernel_size=3, stride=1, padding=1, bias=False)
        self.linear = nn.Linear(64, num_classes)
        self.maxpool2d = nn.MaxPool2d(kernel_size=2, stride=2)
        self.relu = nn.ReLU(inplace=True)
        self.BNend = nn.BatchNorm2d(64)
        self.bypass = nn.Conv2d(self.ocs[-1], 64, kernel_size=1, stride=1, bias=False)
        if self.cls != 0:
            self.b0 = nn.ParameterList([nn.Parameter(torch.zeros(1, 64, 1, 1))])
            self.FBconv = nn.ConvTranspose2d(64, self.ocs[-1], kernel_size=3, stride=1, padding=1, bias=False) 
    ...
    def forward(self, x):
        ...
        # classifier                
        out = self.relu(self.FFconv(x))
        if self.cls != 0:
            b0 = self.relu(self.b0[0] + 1.0).expand_as(out)
            for _ in range(self.cls):
                out = self.FFconv(self.relu(x- self.FBconv(out))) * b0 + out
            
        out = self.BNend(F.max_pool2d(out + self.bypass(x), 2))
        out = self.linear(F.avg_pool2d(out, out.size(-1)).squeeze(-1).squeeze(-1))
