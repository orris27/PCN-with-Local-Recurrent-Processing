...
class PcConvBp(nn.Module):
    def __init__(self, inchan, outchan, kernel_size=3, stride=1, padding=1, bias=False, adaptive=True, cls=0, dropout=1.0):
        super().__init__()
...
class PredNetBpD(nn.Module):
    def __init__(self, num_classes=10, cls=0, dropout=1.0, adaptive=False, vanilla=False, ge=False, fb=None):
        ...
        self.clf_h = [32, 64, 64] # hidden channels for classifiers
        ...
        for i in range(self.nlays):
            self.PcConvs.append(PcConvBp(self.ics[i], self.ocs[i], adaptive=self.adaptive, cls=self.cls, dropout=self.dropout))
